...
class BuildDiskimageCommand(BaseCommand):
    ...
    def do_manifest(self, manifest: dict) -> None:
        ...
        part_swap = part.add(
            PartitionType.SWAP_AMD64,
            uuid4(),
            nr=2,
            size=2 * 1024 * 1024 * 1024,  # 2GB swap
        )
        part_home = part.add(
            PartitionType.HOME_AMD64,
            uuid4(),
            nr=3,
            size=1 * 1024 * 1024 * 1024,  # 1GB home
        )
        part_variable = part.add(
            PartitionType.VARIABLE_AMD64,
            uuid4(),
            nr=4,
            size=1 * 1024 * 1024 * 1024,  # 1GB variable
        )
        part_temp = part.add(
            PartitionType.TEMP_AMD64,
            uuid4(),
            nr=5,
            size=512 * 1024 * 1024,  # 512MB temp
        )
